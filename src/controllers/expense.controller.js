...
const validateAndGenerateItemList = (items) => {
    if (items === undefined) {
        throw new ApiError(400, "Items field is missing.");
    }
    ...
    items.forEach((item) => {
        if (item.name === undefined)
            throw new ApiError(400, "Item's name field is missing.");

        const itemName = item.name.trim();

        if (itemName === "")
            throw new ApiError(400, "Item's name is required.");

        if (item.qty === undefined) {
            throw new ApiError(
                400,
                `Item's qty field is missing for item: ${itemName}.`
            );
        }

        const itemQty = item.qty.trim();
        if (itemQty === "") {
            throw new ApiError(
                400,
                `Item's qty is required for item: ${itemName}.`
            );
        }

        if (isNaN(Number(itemQty)) || Number(itemQty) <= 0) {
            throw new ApiError(
                400,
                `Item's qty should be a number greater than 0 for item: ${itemName}.`
            );
        }
        ...
        if (item.price === undefined) {
            throw new ApiError(
                400,
                `Item's price field is missing for item: ${itemName}.`
            );
        }
        ...
        if (itemPrice === "") {
            throw new ApiError(
                400,
                `Item's price is required for item: ${itemName}.`
            );
        }
        ...
        if (isNaN(Number(itemPrice)) || Number(itemPrice) <= 0) {
            throw new ApiError(
                400,
                `Item's price should be a number greater than 0 for item: ${itemName}.`
            );
        }
...
const addExpense = asyncHandler(async (req, res, next) => {
    try {
        const { clientEmail, dueDate, description, items, status } = req.body;
        ...
        const itemList = validateAndGenerateItemList(items);
        const totalAmount = calculateTotalAmount(itemList);
...
const updateExpense = asyncHandler(async (req, res, next) => {
    try {
        ...
        const update = {
            $set: { dueDate, description, items, totalAmount, status },
        };
